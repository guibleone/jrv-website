...
const Drawer: React.FC<DrawerProps> = ({ width = 270, children }) => {
  ...
  return (
    <React.Fragment>
      ...
      <button
        onClick={toggleOpen}
        className="hamburger absolute top-2 right-4 py-4 block sm:hidden"
      >
        <Menu size={25} />
      </button>
      ...
      <motion.div
        className="fixed top-0 bottom-0 z-30 left-0"
        initial="closed"
        animate={isOpen ? "open" : "closed"}
        variants={variants}
      ...
      >
        ...
        <div
          style={{ width }}
          className="bg-white  p-4 h-full mx-auto select-text"
        >
